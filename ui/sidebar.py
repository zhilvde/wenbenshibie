...
def render(
    ...
    直接读写 st.session_state 中的导出 / 重置状态。
    """
    # ---- 标题 ----
    st.markdown("""
    ...
    # ---- 过滤 ----
    st.markdown(
        '<div class="sb-section">过滤</div>', unsafe_allow_html=True,
    )
    min_freq: int = st.slider("最低词频", 1, 20, 2, help="过滤出现次数低于此值的词语")

    # ---- 图表 ----
    st.markdown(
        '<div class="sb-section">图表</div>', unsafe_allow_html=True,
    )
    ...
    if st.session_state.get("analyzed"):
        st.markdown(
            '<div class="sb-section">导出</div>', unsafe_allow_html=True,
        )
        ...
        # HTML 报告
        ready = st.session_state.get("html_report_ready") and st.session_state.get("html_report_data")
        if not ready:
            if st.button("生成 HTML 报告", use_container_width=True, key="gen_html_btn"):
                ...
                st.session_state["html_report_data"] = build_html_report(
            ...
            if st.button("清除报告缓存", key="clear_html", use_container_width=True):
    ...
    # ---- 工具 ----
    st.markdown(
        '<div class="sb-section">工具</div>', unsafe_allow_html=True,
    )
    ...
    # ---- 使用说明 ----
    st.markdown("""
    <div style="margin-top:16px;padding-top:12px;border-top:1px solid #393b40;
